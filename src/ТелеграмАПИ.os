...
//  МассивВариантовОтвета - Массив - Массив строк с вариантами ответов
...
//  БезОповещения  - Булево - Если Истина, то сообщение будет переслано без оповещения получателя
//  ТипВикторина  - Булево - Если Истина, то опрос будет создан в режиме викторина
//  ИдПравильногоОтвета  - Число - Индекс правильного ответа, только в режиме опроса викторина
//
// Возвращаемое значение:
//   Структура   - Структура, которую дополнительно нужно передать в функцию ДобавитьВариантОтветаНаОпрос()
//
Функция НовыйОпрос(ПолучательИД,
					ТекстВопроса,
					МассивВариантовОтвета,
					Анонимный = Истина,
					ВозможенМножественныйОтвет = Ложь,
					БезОповещения = Ложь,
					ТипВикторина = Ложь,
					ИдПравильногоОтвета = Неопределено) Экспорт
	...
	Опрос.Вставить("type", ?(ТипВикторина, "quiz", "regular"));
	Если ТипВикторина И ИдПравильногоОтвета <> Неопределено Тогда
		Опрос.Вставить("correct_option_id", ИдПравильногоОтвета);
	КонецЕсли;
